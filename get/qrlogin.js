function getqrpic(){
	var getvcurl='get/login.php?do=getqrpic&r='+Math.random(1);
...
function ptuiCB(code,uin,sid,skey,pskey,pskey2,nick){
	...
	switch(code){
		case '0':
			$('#login').html('<br/><div class="alert alert-success">QQ验证成功！'+decodeURIComponent(nick)+'</div><br/><a class="btn" href="/download_get.php?my=updater&qq='+uin+'" target="_blank">下载更新包</a><a class="btn" href="/download_get.php?my=installer&qq='+uin+'" target="_blank">下载安装包</a>');
			$('#qrimg').hide();
			$('#submit').hide();
...
function loadScript(c) {
	...
	c = c || "get/login.php?do=qqlogin&qrsig="+decodeURIComponent(qrsig)+"&r=" + Math.random(1);
