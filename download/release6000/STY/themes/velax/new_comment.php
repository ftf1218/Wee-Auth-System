<?php
/*
 * @FilePath: /STY/themes/velax/new_comment.php
 * @author: Wibus
 * @Date: 2021-08-10 22:51:26
 * @LastEditors: Wibus
 * @LastEditTime: 2021-08-10 22:55:09
 * Coding With IU
 */
?>
    ...
    function threadedComments($comments, $options) {
        ...
        if ($comments->authorId) {
            if ($comments->authorId == $comments->ownerId) {
                ...
                $Identity = '<label data-toggle="tooltip" data-placement="right" title="'._t("作者").'" class="label comment-author-logo m-l-xs"><span><i data-feather="zap"></i></span></label>';
            } else {
                $commentClass .= ' comment-by-user';  //如果是评论作者的添加 .comment-by-user 样式
                //$Identity = '<label class="label bg-dark m-l-xs">'._t("用户").'</label>';
            }
        }else{
            $Identity = '';
        }
    ...
    <div id="comments">
        ...
        <?php if ($this->options->commentPosition == 'bottom' || $this->options->commentPosition ==  ""): ?>
            ...
            Others::returnCommentList($this,$this->security,$comments) ?>
        <?php endif; ?>
            ...
            <div id="<?php $this->respondId(); ?>" class="respond comment-respond no-borders">
                ...
                <h4 id="reply-title" class="comment-reply-title m-t-lg m-b"><?php _me("发表评论") ?>
                    <small><i class="glyphicon glyphicon-info-sign" data-toggle="tooltip" data-placement="right" title="<?php
                        ...
                        if (trim($tip) == ""){
                            $tip = _t("使用cookie技术保留您的个人信息以便您下次快速评论，继续评论表示您已同意该条款");
                        }
                        echo $tip;
                        ?>"></i>
                    </small>
